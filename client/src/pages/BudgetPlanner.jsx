  ...
  useEffect(() => {
    const loadBudget = async () => {
      try {
        ...
        const url = userId ? `${API_BASE}/api/budget?userId=${encodeURIComponent(userId)}` : `${API_BASE}/api/budget`
  ...
  const handleSave = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE}/api/budget`, {
        ...
        body: JSON.stringify({ userId, income, categories })
      })
      const payload = await res.json()
      if (!res.ok || !payload?.success) throw new Error(payload?.error || 'Failed to save budget')
      setIsEditing(false)
