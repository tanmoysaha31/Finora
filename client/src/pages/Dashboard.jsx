  ...
  useEffect(() => {
    ...
    const mockDB = {
      user: {
        ...
        totalBalance: 124592.5
      },
      chartData: {
        labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul'],
        income: [100, 260, 290, 280, 330, 400, 380],
        expense: [120, 70, 170, 290, 100, 150, 210]
      },
      goals: [
                ...
                <div className="glass-panel rounded-3xl p-2 md:p-2.5 flex-1 flex flex-col min-h-0">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-1 md:gap-1.5 flex-1 min-h-0 overflow-y-auto custom-scroll">
                    {loading ? (
                      ...
                      data?.goals?.map(goal => (
                        <div key={goal.id} className="bg-white text-gray-900 rounded-lg p-2 flex justify-between items-center shadow-lg hover-lift cursor-pointer group h-full relative overflow-hidden">
                          {/* Background Pattern */}
                          ...
                          <div className="flex flex-col items-end gap-1.5 relative z-10">
                            <div className={`w-7 h-7 md:w-8 md:h-8 rounded-md ${goal.bg} flex items-center justify-center text-white shadow-md group-hover:scale-110 transition-transform duration-300`}>
                              <i className={`fa-solid ${goal.icon} text-xs md:text-sm`}></i>
                            </div>
                            <div className="text-right">
                              <span className="text-[10px] md:text-xs font-bold text-indigo-600 group-hover:underline">View Details</span>
                            </div>
                          </div>
